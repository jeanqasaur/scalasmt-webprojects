...
//      (__ \ "secretId").read[String] and
//      (__ \ "email").read[String] and
//      (__ \ "_name").read[String] and
//      (__ \ "_affiliation").read[String] and
//      (__ \ "_password").read[String] and
//      (__ \ "_isGrad").read[Boolean] and
//      (__ \ "_acmNum").read[String] and
//      (__ \ "role").read[UserStatus] and
//      (__ \ "_conflicts").read[List[Long]])
//  implicit val userWrite = Json.writes[ConfUser]
//}
//
//object UserStatus {
//  implicit val userStatusR: Reads[UserStatus] = 
//	__.read[AuthorStatus].map(x => x:AuthorStatus) |
//	__.read[PublicStatus].map(x => x:PublicStatus) |
//	__.read[ReviewerStatus].map(x => x:ReviewerStatus) |
//	__.read[PCStatus].map(x => x:PCStatus)
//  
//  implicit val userStatusW: Writes[UserStatus] = Json.obj()
//}
