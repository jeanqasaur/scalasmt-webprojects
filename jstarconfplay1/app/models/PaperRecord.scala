...
//object PaperStage {
//  implicit val paperStageR: Reads[PaperStage] = 
//	__.read[Submission].map(x => x:Submission) |
//	__.read[Review].map(x => x:Review) |
//	__.read[Public].map(x => x:Public)
//  
//  implicit val paperStageW: Writes[PaperStage] = Json.obj()
//}
